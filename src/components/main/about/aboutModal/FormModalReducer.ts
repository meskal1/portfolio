...
export const FormModalReducer = (state: FormModalInitStateType = formModalInitState, action: ActionType): FormModalInitStateType => {
  switch (action.type) {
    case 'ON_CHANGE_COMPANY': {
      let validCompany = action.payload.companyValue.slice(0, 100).trimStart()

      if (validCompany[validCompany.length - 1] === ' ' && validCompany[validCompany.length - 2] === ' ') {
    ...
    case 'ON_CHANGE_CONTACT': {
      ...
      if (validContact[validContact.length - 1] === ' ' && validContact[validContact.length - 2] === ' ') {
